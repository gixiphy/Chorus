...
OSStatus ProxyAudioDevice::Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost) {
    ...
    if (boxName == NULL) {
        boxName = CFStringCreateCopy(NULL, CFSTR("Chorus Audio Box"));
    }
    ...
    audioOutputQueue = dispatch_queue_create("com.hermes.ChorusAudioDevice.audioOutputQueue", priorityAttribute);
    ...
    applyVolumeToSamples = retrieveApplyVolumeFromStorage();
...
#pragma unused(inChangeAction, inChangeInfo)
    ...
    syslog(LOG_ERR,
           "ProxyAudio error: was not able to change the sample rate of Chorus Screen Output to %llu",
           inChangeAction);
    ...
    switch (inAddress->mSelector) {
        ...
        case kAudioObjectPropertyModelName:
            ...
            FailWithAction(inDataSize < sizeof(CFStringRef),
                           ...
                           "GetBoxPropertyData: not enough space for the return value of "
                           "kAudioObjectPropertyManufacturer for the box");
            *((CFStringRef *)outData) = CFSTR("Chorus Audio Model");
            *outDataSize = sizeof(CFStringRef);
            break;
...
void ProxyAudioDevice::parseConfigurationString(CFStringRef configString, ConfigType &action, CFStringRef &value) {
    ...
    } else if (CFStringCompare(actionString, CFSTR("applyVolume"), 0) == kCFCompareEqualTo) {
        action = ConfigType::applyVolume;
    } else {
        return;
    }
...
void ProxyAudioDevice::setConfigurationValue(ConfigType type, CFStringRef value) {
    switch (type) {
        ...
        case ConfigType::applyVolume:
            setApplyVolume(CFStringGetIntValue(value) != 0);
            break;
...
CFStringRef ProxyAudioDevice::copyConfigurationValue(ConfigType type) {
    ...
    switch (type) {
        ...
        case ConfigType::applyVolume:
            return CFStringCreateWithFormat(NULL, NULL, CFSTR("%u"), applyVolumeToSamples ? 1 : 0);
...
CFStringRef ProxyAudioDevice::copyDeviceNameFromStorage()
{
    ...
    if (result == NULL) {
        ...
        result = CFBundleCopyLocalizedString(
            bundle, CFSTR("DeviceName"), CFSTR("Chorus Screen Output"), CFSTR("Localizable"));
    }

    if (result == NULL) {
        result = CFStringCreateCopy(NULL, CFSTR("Chorus Screen Output"));
    }
...
bool ProxyAudioDevice::retrieveApplyVolumeFromStorage() {
    DebugMsg("ProxyAudio: retrieveApplyVolumeFromStorage");
    ...
    CFPropertyListSmartRef data;
    gPlugIn_Host->CopyFromStorage(gPlugIn_Host, CFSTR("applyVolume"), &data);
...
void ProxyAudioDevice::setApplyVolume(bool newApplyVolume) {
    CAMutex::Locker locker(&stateMutex);
    applyVolumeToSamples = newApplyVolume;
    gPlugIn_Host->WriteToStorage(gPlugIn_Host,
                                 CFSTR("applyVolume"),
                                 newApplyVolume ? kCFBooleanTrue : kCFBooleanFalse);
}
